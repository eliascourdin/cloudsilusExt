...
@Accessors
public abstract class AbstractSwitchExt extends CloudSimEntity implements SwitchExt {
    ...
    @Getter
    private final List<SwitchExt> uplinkSwitches;

    @Getter
    private final List<SwitchExt> downlinkSwitches;
     ...
     */
    private final Map<SwitchExt, List<HostPacketExt>> uplinkSwitchPacketMap;
     ...
     */
    private final Map<SwitchExt, List<HostPacketExt>> downlinkSwitchPacketMap;
    ...
    protected void processHostPacket(final SimEvent evt) {
        if(evt.getData() instanceof HostPacketExt pkt) {
            final NetworkHostExt host = pkt.getDestination();
            host.addReceivedNetworkPacket(pkt);
        }
        else throw new IllegalStateException("NETWORK_EVENT_HOST SimEvent data must be a HostPacketExt");
    }
     ...
     */
    protected NetworkHostExt getVmHost(final Vm vm) {
    ...
    private void forwardPacketsToSwitch(
        final SwitchExt destinationSwitch, final List<HostPacketExt> packetList,
        final double bandwidth, final int tag)
    {
        for (final HostPacketExt pkt : packetList) {
    ...
    private void forwardPacketsToHosts() {
        for (final NetworkHostExt host : packetToHostMap.keySet()) {
    ...
    @Override
    public double downlinkTransferDelay(final HostPacketExt packet, final int simultaneousPackets) {
        return packetTransferDelay(packet, downlinkBandwidth, simultaneousPackets);
    }

    @Override
    public double uplinkTransferDelay(final HostPacketExt packet, final int simultaneousPackets) {
    ...
    protected double packetTransferDelay(
        final HostPacketExt netPkt, final double bwCapacity, final int simultaneousPackets)
    {
     ...
     */
    protected List<HostPacketExt> getDownlinkSwitchPacketList(final SwitchExt downlinkSwitch) {
     ...
     * @return the list of packets to be sent to the given Switch.
     */
    protected List<HostPacketExt> getUplinkSwitchPacketList(final SwitchExt uplinkSwitch) {
     ...
     */
    protected List<HostPacketExt> getHostPacketList(final NetworkHostExt host) {
        return packetToHostMap.getOrDefault(host, new ArrayList<>());
    }

    /**
     * Adds a packet that will be sent to a downlink {@link SwitchExt}.
     * @param downlinkSwitch the target Switch
     * @param packet the packet to be sent
     */
    protected void addPacketToSendToDownlinkSwitch(final SwitchExt downlinkSwitch, final HostPacketExt packet) {
        computeMapValue(downlinkSwitchPacketMap, downlinkSwitch, packet);
    }

    protected void addPacketToBeSentToFirstUplinkSwitch(final HostPacketExt netPkt) {
        final SwitchExt uplinkSw = getUplinkSwitches().get(0);
        addPacketToSendToUplinkSwitch(uplinkSw, netPkt);
    }

    /**
     * Adds a packet that will be sent to an uplink {@link SwitchExt}.
     * @param uplinkSwitch the target Switch
     * @param packet the packet to be sent
     */
    protected void addPacketToSendToUplinkSwitch(final SwitchExt uplinkSwitch, final HostPacketExt packet) {
     ...
     * @param packet the packet to be sent
     */
    protected void addPacketToSendToHost(final NetworkHostExt host, final HostPacketExt packet) {
